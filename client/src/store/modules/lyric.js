...
export const actions = {
  ...
  deleteCategory ({ commit, dispatch }, category) {
    ...
    if (toDelete === -1) {
      const alert = {
        message: i18n.t('category.deleteFail'),
        type: 'error'
      }
      dispatch('appState/showAlert', alert, { root: true })
    }
    ...
    return lyricsApi.deleteLyricCategory(category).then(response => {
      ...
      if (response.status === 200) {
        const alert = {
          message: i18n.t('category.deleteSuccess'),
          type: 'success'
        }
        dispatch('appState/showAlert', alert, { root: true })
      } else {
        const alert = {
          message: i18n.t('category.deleteFailDB'),
          type: 'error'
        }
        dispatch('appState/showAlert', alert, { root: true })
      }
